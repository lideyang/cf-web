...
define(function(require, exports, module) {
	...
	(function ($) {
		...
		$.extend(Plugin.prototype, {
						...
						if (data.code == '200') {
							...
							if (typeof (data[0]) == 'string') {
								for (var item in data) {
									str += '<li class="option-item" data-index="' + item + '" data-value="' + data[item] + '">' + data[item] + '</li>';
									str2 += '<option value="' + data[item] + '">' + data[item] + '</option>';
								}
							} else {
								for (var item in data) {
									str += '<li class="option-item" data-index="' + item + '" data-value="' + data[item].linkLoginId + '">' + data[item].linkLoginId + '</li>';
									str2 += '<option value="' + data[item].linkLoginId + '">' + data[item].linkLoginId + '</option>';
								}
							}
							$self.$dropdown.html(str);
							$self.$el.html(str2);
							$self._update();//更新option
							var items = $self._getAll();
							$self._filterItems(items, val);
						} else {
			...
			_filterItems: function (items, search) {
				...
				$('.' + this.settings.markerClass, items).contents().unwrap();
			...
			_filter: function (search) {
				...
				if (this.settings.ajax) {
					this._ajax(this.settings.ajax, search)
				} else {
					var items = this._getAll();
					this._filterItems(items, search);
				}
			},
			...
			_blur: function () {
				...
				var index = this.$options.filter(function () {
					...
					if (isNumber) {
						return parseInt($.trim($(this).text()).toLowerCase()) == val
					}

					return $.trim($(this).text()).toLowerCase() == val
				...
				// console.log($("#payeeUserNo").val());
				// //$("#payeeUserName").val('');
				// //this.$el.trigger("blur");
				var e = $.Event('blur.select')
				this.$el.trigger(e);
			},
			...
			_update: function () {
				//console.log('_update'+$("#payeeUserNo").val());
				this.$options = this.$el.find('option, optgroup')
